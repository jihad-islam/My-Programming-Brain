/*
imp: dubly linked list er insertion er logic ta bujhte hobe. eta DSA note khatay details e figure draw kore bujhano ase.    
    ...
    2. temp->next = newNode             → 20 → 100 // temp er next e newNode er address boshaya dilam.
...
void insert_at_position(Node* head, int position, int value) {
    ...
    // ei duita order must same rakhte hobe
    newNode->next = tmp->next;  
    tmp->next = newNode;        

    // ei duitar order same na rakhlew hobe.
    newNode->next->prev = newNode;
    newNode->prev = tmp;
}
